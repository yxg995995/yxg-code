...
class Ui_in_orderWindow(object):
    def setupUi(self, in_orderWindow):
        in_orderWindow.setObjectName("in_orderWindow")
        in_orderWindow.resize(622, 504)
        self.centralwidget = QtWidgets.QWidget(in_orderWindow)
        ...
        in_orderWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(in_orderWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 622, 23))
        self.menubar.setObjectName("menubar")
        self.menu = QtWidgets.QMenu(self.menubar)
        self.menu.setObjectName("menu")
        in_orderWindow.setMenuBar(self.menubar)
        self.actionadd = QtWidgets.QAction(in_orderWindow)
        self.actionadd.setObjectName("actionadd")
        self.actiondelete = QtWidgets.QAction(in_orderWindow)
        self.actiondelete.setObjectName("actiondelete")
        self.help = QtWidgets.QAction(in_orderWindow)
        ...
        self.retranslateUi(in_orderWindow)
        self.clear.clicked.connect(self.search_line.clear)
        QtCore.QMetaObject.connectSlotsByName(in_orderWindow)
        in_orderWindow.setTabOrder(self.switch_line, self.switchPageButton)
        in_orderWindow.setTabOrder(self.switchPageButton, self.search_line)
        in_orderWindow.setTabOrder(self.search_line, self.feature)
        in_orderWindow.setTabOrder(self.feature, self.searchButton)
        in_orderWindow.setTabOrder(self.searchButton, self.clear)
        in_orderWindow.setTabOrder(self.clear, self.prevButton)
        in_orderWindow.setTabOrder(self.prevButton, self.nextButton)
    ...
    def onSearch(self):
        ...
        q="select * from info_order where "+feature+" like %s"
        query=(q%key)
    ...
    def getTotalRecord(self):
        self.queryModel.setQuery("select * from info_order")
    ...
    def setTableView(self):
        ...
        self.queryModel.setHeaderData(1, Qt.Horizontal, "入库时间")
        self.queryModel.setHeaderData(2, Qt.Horizontal, "经办人")
        self.queryModel.setHeaderData(3, Qt.Horizontal, "联系电话")
        
    def recordQuery(self,limitIndex):
        szQuery = ("select * from info_order limit %s,%s" %(limitIndex , self.PageRecordCount))

    def retranslateUi(self, in_orderWindow):
        _translate = QtCore.QCoreApplication.translate
        in_orderWindow.setWindowTitle(_translate("in_orderWindow", "入库单表"))
        self.prevButton.setText(_translate("in_orderWindow", "前一页"))
        self.nextButton.setText(_translate("in_orderWindow", "后一页"))
        self.label.setText(_translate("in_orderWindow", "前往"))
        self.label_2.setText(_translate("in_orderWindow", "页"))
        self.switchPageButton.setText(_translate("in_orderWindow", "go"))
        self.feature.setItemText(0, _translate("in_orderWindow", "info_orderNo"))
        self.feature.setItemText(1, _translate("in_orderWindow", "info_date"))
        self.feature.setItemText(2, _translate("in_orderWindow", "operator"))
        self.feature.setItemText(3, _translate("in_orderWindow", "phone"))
        self.searchButton.setText(_translate("in_orderWindow", "搜索"))
        self.clear.setText(_translate("in_orderWindow", "更新"))
        self.menu.setTitle(_translate("in_orderWindow", "功能"))
        self.actionadd.setText(_translate("in_orderWindow", "add"))
        self.actionadd.setShortcut(_translate("in_orderWindow", "Ctrl+N"))
        self.actiondelete.setText(_translate("in_orderWindow", "delete"))
        self.help.setText(_translate("in_orderWindow", "help"))

import sys
if __name__ == '__main__':
    app = QApplication(sys.argv)
    product=QtWidgets.QMainWindow()
    win=Ui_in_orderWindow()
    win.setupUi(product)
    product.show()
    sys.exit(app.exec_())
